...
static char *variable_suffix_replace (const char *body,
                                      const char *from_s,
                                      const char *to_s)
{
...
void parse_var_line (char *line, enum variable_origin origin)
{
    ...
    if (opt == VAR_APPEND) {
        if (var) {
            /* Flavor is preserved and the appended value
             * is processed as if it was present in the initial definition,
             * so:
             * var1=$(var2)
             * var1+=$(var3)
             * is the same as:
            ...
            /* If the variables was not previously defined,
             * "+=" means the same as "=".
             */
            opt = VAR_ASSIGN;
        }
    ...
    if (opt == VAR_ASSIGN || opt == VAR_CONDITIONAL_ASSIGN) {
        ...
        char *tmp;
        size_t len1 = strlen (var->value);
        size_t len2 = strlen (new_value);

        tmp = xmalloc (len1 + 1 + len2 + 1);
        memcpy (tmp, var->value, len1);
        tmp[len1] = ' ';
        memcpy (tmp + len1 + 1, new_value, len2);
        tmp[len1 + 1 + len2] = '\0';
