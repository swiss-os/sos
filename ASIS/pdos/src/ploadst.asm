...
; Any variable will do, so we just point to the end of the data
dgaddr dd dummy_data
nextzz:

; extract the last 2 bytes which are the segment portion
mov dx,word ptr dgaddr + 2

; Unfortunately as86/ld86 is putting a flat address in the
; above variable, which prevents us finding out the segment.
; So use the old code for now.
ifdef AS86
mov dx, DGROUP
endif
