...
ifdef MASM
; MASM 5.1 can't handle the above rrr address
; And it apparently can't handle "seg" in 32-bit code
; So we hope that the 32-bit offset has what we need

;        mov dx, seg _TEXT32
        mov eax, offset _TEXT32
        mov edx, eax
        shr edx, 16
else
...
; Don't move a 32-bit offset into ebx, so as to reduce
; pressure on the linker to deal with both 32-bit segmented
; fixups and 32-bit flat offsets
;        mov ebx, offset rawprota_stage3
        xor ebx,ebx
        mov bx, offset rawprota_stage3
        jmp ptor
