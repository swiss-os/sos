...
/* Supporting 12 pointer declarators in a single declaration is required
 * but using typedef infinite pointer depth must be supported.
 * ("typedef void ********a; a********b;" is valid and must be supported.)
 * +++The latter is currently not supported.
 */
#define MAX_PTR_DEPTH 12
struct cc_type {
    int tree_type;
    cc_type_mode mode;
    struct cc_ptrinfo {
