...
static size_t cc_i370gen_push(cc_reader *reader, const cc_expr *expr)
{
    ...
    case CC_EXPR_STRING:
        ...
        tprintf(reader->output, "\tST\t%d,%d(,13)\n", CC_REGTYPE_TEMP1, 88 + stack_size);
        return 4;
...
static void cc_i370gen_prologue(cc_reader *reader, const cc_expr *expr)
{
    ...
    fprintf(reader->output, "\tL\t15,76(,13)\n");
    fprintf(reader->output, "\tST\t13,4(,15)\n");
    fprintf(reader->output, "\tST\t15,8(,13)\n");
    fprintf(reader->output, "\tLR\t13,15\n");
    fprintf(reader->output, "\tA\t15,=A(96)\n");
    fprintf(reader->output, "\tST\t15,76(,13)\n");
    stack_size = 0;
}
...
static void cc_i370gen_epilogue(cc_reader *reader, const cc_expr *expr)
{
    ...
    {
        tprintf(reader->output, "\tL\t13,4(,13)\n");
        tprintf(reader->output, "\tL\t14,12(,13)\n");
        tprintf(reader->output, "\tLM\t0,12,20(13)\n");
        tprintf(reader->output, "\tBR\t14\n");
...
static void cc_i370gen_top(cc_reader *reader, const cc_expr *expr)
{
    ...
    switch (expr->type)
    {
        ...
        case CC_EXPR_CALL:
            ...
            fprintf(reader->output, "\tLA\t1,88(,13)\n");
