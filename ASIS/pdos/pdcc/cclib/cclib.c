...
/* cc_parse_declspecs() is interested in st_type and type is only temporary,
 * so this returns st_type.
 */
static cc_type *cc_parse_struct_member_decl (cc_reader *reader, cc_type *type)
{
...
static cc_expr cc_parse_assignment_expr(cc_reader *reader)
{
    /* First, parse the left-hand side (the 'c' in 'c = b') */
    cc_expr left = cc_parse_binary_expr(reader);

    /* Check if the next token is an assignment operator '=' */
    if (reader->curr_token->type == CC_TOKEN_ASSIGN)
    {
        cc_expr expr = {0};

        expr.type = CC_EXPR_ASSIGN;
        expr.id = cc_get_unique_id(reader);

        /* Consume the '=' token so the parser can move forward */
        cc_consume_token(reader);

        /* Allocate and store the left-hand side */
        expr.data.binary_op.left = xmalloc(sizeof(cc_expr));
        *expr.data.binary_op.left = left;

        /* Recursively parse the right-hand side (the 'b' in 'c = b') */
        /* This allows for chained assignments like a = b = c; */
        expr.data.binary_op.right = xmalloc(sizeof(cc_expr));
        *expr.data.binary_op.right = cc_parse_assignment_expr(reader);

        return expr;
    }

    /* If no '=', just return the expression we already parsed */
    return left;
}
...
static cc_type *cc_parse_struct_union_enum (cc_reader *reader)
{
    cc_type type = {CC_TREE_TYPE};
     ...
     * <struct|union|enum> <identifier> { <members> } <var-expr>; */
    if (reader->curr_token->type == CC_TOKEN_LBRACE)
        return cc_parse_struct_member_decl (reader, &type);

    {
        cc_type *copy = pxmalloc (sizeof type);
        *copy = type;
        return copy;
    }
}

static int is_keyword (cc_token_type type)
{
    if (type >= CC_TOKEN_KW && type < CC_TOKEN_EOF) return 1;
    return 0;
}
...
typedef struct cc_declspecs {
    enum type_decide type_decide;
    int storage_class;
...
static void declspecs_add_storage_class (cc_reader *reader,
                                         cc_declspecs *specs,
                                         cc_token_type type)
{
#define CC_TOKEN_NONE 0 /* Currently CC_TOKEN_ASSIGN but that is fine because it cannot be used here.*/
    if (specs->storage_class != CC_TOKEN_NONE) {
        if (specs->storage_class == type) {
            cc_report (reader, CC_DL_ERROR,
        ...
        specs->storage_class = type;
    }
}

static void declspecs_add_type_tree (cc_reader *reader,
                                     cc_declspecs *specs,
                                     struct cc_type *tree)
{
    specs->type = tree;
    specs->type_decide = TYPE_DECIDE_OTHER;
}

static void declspecs_add_type (cc_reader *reader,
                                cc_declspecs *specs,
                                cc_token_type type)
{
    /* +++Add checks to exclude 'short' and 'long', 'long' and 'void', 'void' and 'int' etc. */
    if (specs->type) {
        cc_report (reader, CC_DL_ERROR, "two or more data types in declaration specifiers");
    }
    switch (type) {
        case CC_TOKEN_KW_SHORT:
            specs->short_yes = 1;
        ...
        case CC_TOKEN_KW_LONG:
            if (specs->long_long_yes) {
                cc_report (reader, CC_DL_ERROR,
                           "'long long long' is not yet supported");
            }
            if (specs->long_yes) {
                specs->long_long_yes = 1;
            }
            specs->long_yes = 1;
            break;
        ...
        case CC_TOKEN_KW_UNSIGNED:
            specs->unsigned_yes = 1;
            break;
        
        case CC_TOKEN_KW_VOID:
            specs->type_decide = TYPE_DECIDE_VOID;
            break;
        
        case CC_TOKEN_KW_CHAR:
            specs->type_decide = TYPE_DECIDE_CHAR;
            break;
        
        case CC_TOKEN_KW_INT:
            specs->type_decide = TYPE_DECIDE_INT;
            break;
        
        case CC_TOKEN_KW_FLOAT:
            specs->type_decide = TYPE_DECIDE_FLOAT;
            break;

        case CC_TOKEN_KW_DOUBLE:
            specs->type_decide = TYPE_DECIDE_DOUBLE;
            break;
    }
}

static void declspecs_add_qualifier (cc_reader *reader,
                                     cc_declspecs *specs,
                                     cc_token_type type)
{
    if (type == CC_TOKEN_KW_CONST) {
        if (specs->const_yes) cc_report (reader, CC_DL_WARNING, "Redundant qualifier");
        specs->const_yes = 1;
    } else if (reader->curr_token->type == CC_TOKEN_KW_VOLATILE) {
        if (specs->volatile_yes) cc_report (reader, CC_DL_WARNING, "Redundant qualifier");
        specs->volatile_yes = 1;
    }
}

static void declspecs_finish (cc_reader *reader,
                              cc_declspecs *specs)
{
    /* +++The correct way to do this is to have already prepared built-in types
     * (such as one for 'unsigned long long int') and then choose
     * based on type_decide and long_long_yes and unsigned_yes etc.
     * a pointer to unsigned_long_long_int_tree for specs->type
     * instead of generating new types every time.
     */
    cc_type type = {CC_TREE_TYPE};

    if (specs->type) return;
    
    if (specs->const_yes) type.ptr[type.ptr_depth].flags |= CC_PTRINFO_CONST;
    if (specs->volatile_yes) type.ptr[type.ptr_depth].flags |= CC_PTRINFO_VOLATILE;
    type.ptr_depth++;

    if (specs->type_decide == TYPE_DECIDE_NONE) {
        if (specs->short_yes || specs->long_yes
            || specs->signed_yes || specs->unsigned_yes) {
            specs->type_decide = TYPE_DECIDE_INT;
        } else {
            cc_report (reader, CC_DL_WARNING, "type defaults to 'int'");
            specs->type_decide = TYPE_DECIDE_INT;
        }
    }

    switch (specs->type_decide) {
        case TYPE_DECIDE_VOID:
            type.mode = CC_TYPE_VOID;
            break;
        
        case TYPE_DECIDE_CHAR:
            type.mode = CC_TYPE_CHAR;
            break;
        
        case TYPE_DECIDE_INT:
            if (specs->long_long_yes) {
                type.mode = CC_TYPE_LLONG;
            } else if (specs->long_yes) {
                type.mode = CC_TYPE_LONG;
            } else if (specs->short_yes) {
                type.mode = CC_TYPE_SHORT;
            } else {
                type.mode = CC_TYPE_INT;
            }
        ...
        case TYPE_DECIDE_FLOAT:
            type.mode = CC_TYPE_FLOAT;;
            break;
        
        case TYPE_DECIDE_DOUBLE:
            type.mode = CC_TYPE_DOUBLE;
            break;
        
    }

    if (specs->signed_yes) type.data.sign = 1;
    if (specs->unsigned_yes) type.data.sign = 0;

    specs->type = pxmalloc (sizeof type);
    *specs->type = type;
}

/* Handles declaration specifiers - the first part ("int") of "int *abc;".
 * Pointers ('*') are part of declarator and should be handled separately.
 */
static void cc_parse_declspecs (cc_reader *reader, cc_declspecs *specs)
{
    int seen_type = 0;

    while (cc_peek_token (reader)->type == CC_TOKEN_IDENT
           || is_keyword (cc_peek_token (reader)->type)) {
        if (cc_peek_token (reader)->type == CC_TOKEN_IDENT) {
            cc_token *token;
            
            if (seen_type) return;

            token = cc_peek_token (reader);
            search_scope (token);
            if (token->id_kind == CC_ID_ID) return;

            seen_type = 1;
            declspecs_add_type_tree (reader, specs, token->decl);
            cc_consume_token (reader);
            continue;
        }
        ...
        switch (cc_peek_token (reader)->type) {
            ...
            case CC_TOKEN_KW_STATIC:
            case CC_TOKEN_KW_TYPEDEF:
            case CC_TOKEN_KW_INLINE:
                declspecs_add_storage_class (reader, specs, cc_peek_token (reader)->type);
                cc_consume_token (reader);
                break;
            ...
            case CC_TOKEN_KW_UNSIGNED:
                seen_type = 1;
                declspecs_add_type (reader, specs, cc_peek_token (reader)->type);
                cc_consume_token (reader);
                break;
            ...
            case CC_TOKEN_KW_ENUM:
                seen_type = 1;
                declspecs_add_type_tree (reader, specs, cc_parse_struct_union_enum (reader));
                break;

            case CC_TOKEN_KW_CONST:
            case CC_TOKEN_KW_VOLATILE:
                declspecs_add_qualifier (reader, specs, cc_peek_token (reader)->type);
                cc_consume_token (reader);
                break;
        }
    }
...
static cc_variable cc_parse_variable (cc_reader *reader, cc_declspecs *specs)
{
    cc_variable var = {CC_TREE_VAR};
    var.linkage = CC_LINKAGE_AUTO;
    
    if (specs->storage_class == CC_TOKEN_KW_EXTERN) {
        var.linkage = CC_LINKAGE_EXTERN;
    } else if (specs->storage_class == CC_TOKEN_KW_STATIC) {
        var.linkage = CC_LINKAGE_STATIC;
    } else if (specs->storage_class == CC_TOKEN_KW_INLINE){
        var.linkage = CC_LINKAGE_INLINE;
    } else if (specs->storage_class == CC_TOKEN_KW_AUTO) {
        var.linkage = CC_LINKAGE_AUTO;
    }

    /* Now get the type. */
    {
        cc_type type = *specs->type;
...
static cc_expr cc_parse_declaration_or_fndef (cc_reader *reader, int fndef_allowed)
{
    ...
    cc_parse_declspecs (reader, &specs);
    declspecs_finish (reader, &specs);

    if (specs.storage_class == CC_TOKEN_KW_TYPEDEF) {
        cc_type s_type;

        {
            cc_type type = *specs.type;

            /* Pointers of the form <*> <cv> may come now after declaration specifiers. */
            while(1)
            {
                if (reader->curr_token->type == CC_TOKEN_ASTERISK)
                {
...
#ifdef __CC64__
#else
        s_var = cc_parse_variable (reader, &specs);
        if (s_var.name == NULL) {
