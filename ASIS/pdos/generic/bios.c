...
#include <setjmp.h>
    ...
    || defined(LINDIR)
int __close(int fd);
int __open(const char *path, int flags, int x);
int __chdir(const char *filename);
int __mkdir(const char *filename, int x);
int __rmdir(const char *filename);

/* note that MacOS doesn't have a getcwd.
   So we have a really horrible kludge that
   gives you one shot at getting to where
   you need to go, and then you have to exit.
   We probably need to bite the bullet and
   use that /usr/lib/libSystem.B.dylib thing. */
...
static DTA_BIOS *init_ldta(void);
static void free_ldta(void);
static DTA_BIOS *get_dta(void);
static void dtaClean(DTA *dta);
static void dtaCopyData(DTA *dst, DTA *src);
static void dtaBiosCp(void *dst, void *src, int sztype);
static int dtaBiosCmp(void *dst, void *src, int sztype);
static void dtaBiosClean(DTA_BIOS *dta);
#endif
...
int main(int argc, char **argv)
{
...
#if defined(__MACOS__) || defined(__gnu_linux__) && !defined(__M68K__)
#if defined(__MACOS__)
    strcpy(__cwd, ".");
#elif defined(__gnu_linux__) && !defined(__M68K__) /* || defined(__ARM__) */
    __getcwd(__cwd, FILENAME_MAX);
#endif
    init_ldta();
#endif
    ...
    && !defined(__EFI__)
...
#else
        {
            ...
            if (p != NULL)
            {
                ret = PosFindFirst((char *)(p + 1), 0x10);
            }
            else
            {
                /**
                 * Default value is '*' to list all file and folder,
                 * list all files only, please use '*.*'
                 */
...
/* cc64 uses lots of BSS - 47 MB - so on any environment that
   ...
   may as well just let exeload do that on those environments */
    ...
    if (salone)
    {
        ...
        __gcmd = cmd; /* start will use this and then set it to NULL */
        rc = __start(cmd); /* we should start passing NULL instead of cmd */
        salone = 0;
        __genmain = 0;
    }
    else
...
#if 0
/*
MacOS looks like this, and fills the whole buffer up,
and returns 14
000000  349EAD00 0C000401 2E000000 B09DAD00  4...............
000010  0C000402 2E2E0000 5C9EAD00 1800080C  ........\.......
000020  6D616B65 62696F73 2E756136 00000000  makebios.ua6....
...
000120  62696F73 2E73686D 00000000 8A9EAD00  bios.shm........
000130  10000806 70646F73 2E630000 779EAD00  ....pdos.c..w...
000140  1800080C 6D616B65 636F6D6D 2E773634  ....makecomm.w64
...
0001C0  00000000 17E1AE00 0C000403 78783300  ............xx3.
0001D0  609EAD00 1800080C 6D616B65 62696F73  `.......makebios
0001E0  2E773332 00000000 F2579704 01000000  .w32.....W......
0001F0  00000000                             ....
(this is the end)
(and xx3 is a directory)
So the byte before the filename is a length
And the byte before that is probably 4 for directory,
8 for normal file
So that leaves 10 bytes unknown
I'm guessing it is:
4 bytes for an inode number
2 bytes for the length of the whole structure, including
  both the inode and the length itself
1 byte directory attribute
1 byte filename length
filename, don't know if it is always NUL-terminated
and the last entry with a non-sensical length of 1
presumably means end of buffer.

And when you've reached the very end of multiple calls, you get:

000170  2E65726D 00000000 4D9EAD00 1800080C  .erm....M.......
000180  6D616B65 62696F73 2E6C6E78 00000000  makebios.lnx....
000190  F36FAE00 1800080C 6D616B65 62696F73  .o......makebios
0001A0  2E6D7837 00000000 00000000 00000000  .mx7............
...
At least if you memset the buffer first.

If you memset to X, you get:

000190  F36FAE00 1800080C 6D616B65 62696F73  .o......makebios
0001A0  2E6D7837 00000000 58585858 58585858  .mx7....XXXXXXXX
0001B0  58585858 58585858 58585858 58585858  XXXXXXXXXXXXXXXX
0001C0  58585858 58585858 58585858 58585858  XXXXXXXXXXXXXXXX

So it looks to me like there is no choice but to do
the memset and pick up on that.

Maybe the syscall sets the carry flag or something.

It's also like that at the end of a middle block:

0001C0  1800080C 6D616B65 62696F73 2E766236  ....makebios.vb6
0001D0  00000000 879EAD00 14000809 6D767370  ............mvsp
0001E0  622E6261 74000000 58585858 58585858  b.bat...XXXXXXXX
0001F0  58585858                             XXXX

So I suggest keep reading until you have a block of
all-NULs.

*/
#endif
static int ff_search(DTA_BIOS *dta_bios)
{
#define BYTE_IGNORE 4
#define BYTE_LEN 2
#define BYTE_ATTR 1
#define BYTE_FNAME_LEN 1

    char *pstart;
    char *pcheck;

    pcheck = dta_bios->m_buf + dta_bios->m_upto;
    if (dta_bios->m_avail <= 0 ||
        dta_bios->m_avail == dta_bios->m_upto ||
        pcheck[0] == '\0')
    {
        dta_bios->m_upto = 0;
        memset(dta_bios->m_buf, '\0', BYTE_READ);
        dta_bios->m_avail = __getdents(dta_bios->m_fd, dta_bios->m_buf, BYTE_READ);
        if (dta_bios->m_avail != 14)
        {
            dta_bios->m_avail = 0;
            dta_bios->m_upto = 0;
            dta_bios->m_inuse = 0;
            __close(dta_bios->m_fd);
            return (1);
        }
        dta_bios->m_avail = BYTE_READ;
    }
    
    pcheck = dta_bios->m_buf + dta_bios->m_upto;
    while (pcheck[0] != '\0' && dta_bios->m_upto < dta_bios->m_avail)
    {
        int len;
        unsigned short attr;
        char *pnext;

        dtaClean(&dta_bios->m_dta);
        pstart = dta_bios->m_buf + dta_bios->m_upto;
        pnext = pstart + BYTE_IGNORE;
        len = pnext[1] << 8 | pnext[0];
        pnext += BYTE_LEN;

        attr = (unsigned short)pnext[0];
        if (attr == 4)
        {
            dta_bios->m_dta.attrib = FILE_ATTR_DIRECTORY;
        }
        else
        {
            dta_bios->m_dta.attrib = 0;
        }
        pnext += BYTE_ATTR;
        pnext += BYTE_FNAME_LEN;

        strncpy((char *)dta_bios->m_dta.file_name, pnext, DTA_FILENAME);
        dta_bios->m_dta.file_name[DTA_FILENAME - 1] = '\0';

        strncpy((char *)dta_bios->m_dta.lfn, pnext, DTA_LFN);
        dta_bios->m_dta.lfn[DTA_LFN - 1] = '\0';

        dta_bios->m_upto += len;
        if (patmat((char *)dta_bios->m_dta.file_name, dta_bios->m_pattern) ||
            patmat((char *)dta_bios->m_dta.lfn, dta_bios->m_pattern))
        {
            /*
             Do not use dtaCopyData here.
             Because the address of the current DTA_PDOS
             is stored in the original DTA.
             So we only copy the field that we need.
            */
            origdta.attrib = dta_bios->m_dta.attrib;
            strcpy(origdta.file_name, dta_bios->m_dta.file_name);
            strcpy((char*)origdta.lfn, (const char*)dta_bios->m_dta.lfn);
            return 0;
        }

        if (dta_bios->m_upto >= dta_bios->m_avail)
        {
            dta_bios->m_upto = dta_bios->m_avail = 0;
            memset(dta_bios->m_buf, '\0', BYTE_READ);
            dta_bios->m_avail = __getdents(
                dta_bios->m_fd, dta_bios->m_buf, BYTE_READ);
...
/*
 Linux looks like as this:
 __getdents:
 000000  11000000 01000000 10002E00 00000004  ................
 000010  01000000 04000000 10002E2E 00000004  ................
 000020  F3BE0000 05000000 10003100 00000008  ..........1.....
 000030  F4BE0000 06000000 10003132 00000008  ..........12....
 000040  F5BE0000 07000000 10003132 33000008  ..........123...
 000050  F6BE0000 08000000 10003132 33340008  ..........1234..
 000060  F7BE0000 09000000 14003132 33343500  ..........12345.
 000070  00000008 F8BE0000 0A000000 14003132  ..............12
 000080  33343536 00000008 F9BE0000 0B000000  3456............
 000090  14003132 33343536 37000008 FABE0000  ..1234567.......
 0000A0  0C000000 14003132 33343536 37380008  ......12345678..
 0000B0  FBBE0000 0D000000 18003132 33343536  ..........123456
 0000C0  37383900 28FA7F08 FCBE0000 0E000000  789.(...........
 0000D0  10004100 00000004 FDBE0000 0F000000  ..A.............

 8 bytes for an inode number
 2 bytes for the length of the whole structure, including
   both the inode and the length itself
 n bytes for file name util reached NULL-terminated.
 n bytes for extra padding
 1 byte directory attribute

 __getdents64:
 000000  17000000 00000000 01000000 00000000  ................
 000010  1800042E 00000000 11000000 00000000  ................
 000020  04000000 00000000 1800042E 2E000000  ................
 000030  F3BE0000 00000000 05000000 00000000  ................
 000040  20000831 2E747874 00000000 00000000   ..1.txt........
 000050  F4BE0000 00000000 06000000 00000000  ................
 000060  20000831 322E7478 74000000 00000000   ..12.txt.......
 000070  F5BE0000 00000000 07000000 00000000  ................
 000080  20000831 32332E6C 6F670000 00000000   ..123.log......
 000090  F6BE0000 00000000 08000000 00000000  ................
 0000A0  20000831 3233342E 63000000 00000000   ..1234.c.......
 0000B0  F7BE0000 00000000 09000000 00000000  ................
 0000C0  20000831 32333435 2E6C6F67 00000000   ..12345.log....
 0000D0  F8BE0000 00000000 0A000000 00000000  ................
 0000E0  20000831 32333435 36000000 00000000   ..123456.......
 0000F0  F9BE0000 00000000 0B000000 00000000  ................
 000100  20000831 32333435 36372E63 00000000   ..1234567.c....
 000110  FABE0000 00000000 0C000000 00000000  ................
 000120  20000831 32333435 36373800 00000000   ..12345678.....
 000130  FBBE0000 00000000 0D000000 00000000  ................
 000140  20000831 32333435 36373839 00000000   ..123456789....
 000150  FCBE0000 00000000 0E000000 00000000  ................
 000160  18000441 00000000 FDBE0000 00000000  ...A............

 16 bytes for an inode number
 2 bytes for the length of the whole structure, including
   both the inode and the length itself
 1 byte directory attribute
 n bytes for file name util reached NULL-terminated.
...
static int ff_search(DTA_BIOS *dta_bios)
{
#if defined(__64BIT__) && (defined(__ARM__) || defined(__LOONG__))
#define __getdents __getdents64
#define BYTE_IGNORE 16
#else
#define BYTE_IGNORE 8
#endif
#define BYTE_LEN 2
#define BYTE_ATTR 1
    ...
    while (dta_bios->m_upto < dta_bios->m_avail)
    {
        int len;
        unsigned short attr;
        char *pnext;
        ...
        pnext = pstart + len - 1;
        attr = (unsigned short)pnext[0];
        pnext = pstart + BYTE_IGNORE + BYTE_LEN;
#endif
        strncpy((char *)dta_bios->m_dta.file_name, pnext, DTA_FILENAME);
        dta_bios->m_dta.file_name[DTA_FILENAME - 1] = '\0';
        strncpy((char *)dta_bios->m_dta.lfn, pnext, DTA_LFN);
...
int PosFindFirst(char *pat, int attrib)
{
    ...
    char pattern[POS_MAX_PATT];
    DTA_BIOS *cur_dta;

    fullname[0] = '\0';
    rc = formatPath(pat, fullname);
    if (rc)
        return (rc);
...
int PosChangeDir(const char *to)
{
    ...
    while (1)
    {
        if (*start != ' ' && *end != ' ')
        {
            strncpy(path, start, (end + 1 - start));
            break;
        }
        if (*start == ' ') ++start;
        if (*end == ' ') --end;
    ...
    rc = formatPath(path, dname);
    
    if (rc == 0) rc = __chdir(dname);

    if (rc == 0)
    {
#ifdef __MACOS__
        /* this provides very limited single shot
           traversal. which is better than a hole in the
        ...
        else
        {
            ...
            p = strrchr(dname, '/');
            if (p == NULL) p = "";
            strncat(__cwd, p, sizeof __cwd);
            __cwd[sizeof __cwd - 1] = '\0';
        }
#else
        __getcwd(__cwd, FILENAME_MAX);
#endif
...
#define return_Status_if_fail(func) \
    do { \
        if ((Status = (func))) \
            { return Status; }} \
    while (0)
...
static EFI_STATUS dir_list (EFI_FILE_PROTOCOL *dir)
{
    ...
    /* Just enough space for 5 CHAR16 FileName for demonstration. */
    buf_size = sizeof (*Buffer) + 8;

    return_Status_if_fail (
        __gBS->AllocatePool (EfiLoaderData, buf_size, (void **)&Buffer));

    while (1) {
        Read = buf_size;
        Status = dir->Read (dir, &Read, Buffer);
        if (STATUS_IS_ERROR (Status) && 
            STATUS_GET_CODE (Status) == EFI_BUFFER_TOO_SMALL) {
        ...
        return_Status_if_fail (
            __gST->ConOut->OutputString (__gST->ConOut, Buffer->FileName));
...
static EFI_STATUS directory_test (void)
{
    ...
    return_Status_if_fail (
        __gBS->HandleProtocol (__gIH, &li_guid, (void **)&li_protocol));

    return_Status_if_fail (
        __gBS->HandleProtocol (
            li_protocol->DeviceHandle,
            &sfs_protocol_guid, (void **)&sfs_protocol
        )
    );
    ...
    if (len != 0) 
    {
        ...
        Status = EfiRoot->Open (
            EfiRoot, &dir, path, OpenModeDirReadWrite, Attributes);
...
static EFI_STATUS check_path(unsigned char *path)
{
    ...
    Status = EfiRoot->Open (
        EfiRoot, &dir, path_check, OpenModeDirReadWrite, Attributes);
...
#if defined(__gnu_linux__) && !defined(__M68K__) /* || defined(__ARM__) */
static int formatPath(const char *input, char *output)
{
    char *p, *q, *k, *z;
    int n, len;
...
static void dtaBiosCp(void *dst, void *src, int sztype)
{
    DTA_BIOS *dst1 = (DTA_BIOS *)dst;
    DTA_BIOS *src1 = (DTA_BIOS *)src;
    dst1->m_fd = src1->m_fd;
    dst1->m_upto = src1->m_upto;
    dst1->m_inuse = src1->m_inuse;
    strcpy(dst1->m_buf, src1->m_buf);
...
static DTA_BIOS *init_ldta(void)
{
    if (g_ldta == NULL)
    {
        g_ldta = list_init(sizeof(DTA_BIOS), (cpy_cb_fn)dtaBiosCp, (cmp_cb_fn)dtaBiosCmp);
    }
}
...
         Then we add a new item to the list and return it.
        ...
        if (item == NULL)
        {
             ...
             */
            cur_dta = (DTA_BIOS*)g_ldta->m_last->m_data;
        }
    }
    else
    {
