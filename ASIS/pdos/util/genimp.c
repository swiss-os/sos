...
/* converts a file like msvcrt.wat:
...
into a .lib, with a command like:
...
int main (int argc, char **argv)
{
    ...
    if (argc != 3 && argc != 4) {
        printf ("usage: %s input_file output_file (optional) x64/i386/unknown\n", argv[0]);
        return 0;
    }
    ...
    if (argc == 4) {
        ...
        } else if (!strcmp (argv[3], "unknown")) {
            wanted_Machine = IMAGE_FILE_MACHINE_UNKNOWN;
        }
    }
