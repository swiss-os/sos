...
/* is this a 64-bit executable - mainly for use on a 32-bit
   environment  with a shim */
int exe64 = 0;
...
#define NO_DLLENTRY 1
    ...
    && !defined(__64BIT__) && !defined(NOKERNEMUL)
#define W32EMUL
#endif
...
#if defined(ZSHIM)
#define ZSTDTHUNK(x) { #x, (unsigned long)g ## x },
#define ZSHIMTHUNKLIST \
ZSTDTHUNK(printf)
#endif
...
#if defined(W64HACK) || defined(W64DLL) || defined(W64EMUL) \
    || defined(WINNEWMOD)
#define STDW32THUNK(x) { #x, (void *)x },
#else
#define STDW32THUNK(x) { #x, (unsigned long)x },
#endif

#define STDW32THUNKLIST \
STDW32THUNK(CloseHandle) \
STDW32THUNK(CreateFileA) \
STDW32THUNK(FindClose) \
STDW32THUNK(FindFirstFileA) \
STDW32THUNK(FindNextFileA) \
STDW32THUNK(CreateDirectoryA) \
STDW32THUNK(GetFileAttributesA) \
STDW32THUNK(CreateProcessA) \
STDW32THUNK(DeleteFileA) \
STDW32THUNK(ExitProcess) \
STDW32THUNK(GetCommandLineA) \
STDW32THUNK(GetConsoleMode) \
STDW32THUNK(GetEnvironmentStrings) \
STDW32THUNK(GetExitCodeProcess) \
STDW32THUNK(GetLastError) \
STDW32THUNK(GetStdHandle) \
STDW32THUNK(GetSystemTime) \
STDW32THUNK(GlobalAlloc) \
STDW32THUNK(VirtualAlloc) \
STDW32THUNK(GlobalFree) \
STDW32THUNK(VirtualFree) \
STDW32THUNK(MoveFileA) \
STDW32THUNK(ReadFile) \
STDW32THUNK(SetConsoleMode) \
STDW32THUNK(SetFilePointer) \
STDW32THUNK(WaitForSingleObject) \
STDW32THUNK(WriteFile) \
STDW32THUNK(GetModuleHandleA) \
STDW32THUNK(GetEnvironmentStringsA)
                         ...
                         unsigned long e_lfanew)
{
    ...
    IMAGE_DATA_DIRECTORY *Pe32_Image_data;
    U32 numrva;
    ...
    if ((coff_hdr.Machine != IMAGE_FILE_MACHINE_AMD64)
        && (coff_hdr.Machine != IMAGE_FILE_MACHINE_LOONGARCH64)
...
#if (TARGET_64BIT && !SHIMCM32) || ZSHIM
...
#else
    else /* zshim will try 32-bit now */
#endif
#endif
    ...
    if (exe64)
    {
    /* the lower 32 bits of the address are in the "BaseOfData"
       field */
    optional_hdr->ImageBase
        = (void *)flip4((unsigned long)optional_hdr->BaseOfData);
    }
    else
#endif
    {
    ...
    Pe32_Image_data = (IMAGE_DATA_DIRECTORY *)(optional_hdr + 1);
#ifdef ZSHIM
    if (exe64)
    {
        /* this is a hack. we should really have both PE32 and
           PE32+ structures defined, and use the appropriate one */
        /* But really - we should be using a 64-bit compiler,
           so nothing is being done for now */
        /* The extra fields in a PE32+ increase the size of the
           structure by 16 bytes */
        Pe32_Image_data = (void *)((char *)Pe32_Image_data + 0x10);
        numrva = *(U32 *)
                 ((char *)optional_hdr
                          + offsetof(Pe32_optional_hdr, NumberOfRvaAndSizes)
                          + 0x10);
    }
    else
#endif
    {
        numrva = optional_hdr->NumberOfRvaAndSizes;
    }
    ...
    if (exeStart != (void *)(ptrdiff_t)optional_hdr->ImageBase)
    {
        ...
        if (numrva > DATA_DIRECTORY_REL)
        {
            IMAGE_DATA_DIRECTORY *data_dir = Pe32_Image_data
                                             + DATA_DIRECTORY_REL;
            ...
            for (; rel_block < end_rel_block;)
            {
                ...
                for (; cur_rel < end_rel; cur_rel++)
                {
                    ...
                    *cur_rel = flip2(*cur_rel);
                    ...
                    {
                        /* I don't think this entire lower stuff is
                           necessary. Normal maths should sort this out!
                           For all locations. */
                        if (lower_exeStart)
                            *(unsigned char **)rel_target -= image_diff;
                        else *(unsigned char **)rel_target += image_diff;
                    ...
                    {
                        /* we need to skip the first 4 bytes, which are
                           required to be zero */
                        rel_target += 4;
                        if (lower_exeStart)
                            *(unsigned char **)rel_target -= image_diff;
    ...
    if (numrva > DATA_DIRECTORY_IMPORT_TABLE)
    {
        IMAGE_DATA_DIRECTORY *data_dir = Pe32_Image_data
...
#if defined(W64HACK) || defined(W64DLL) || defined(W64EMUL) \
    || (defined(WINNEWMOD) && defined(__64BIT__))
                if (strcmp((char *)exeStart + import_desc->Name, "msvcrt.dll") == 0)
                {
                void **thunk;
...
#if defined(NEEDW32KEMUL)
                else if (strcmp((char *)exeStart + import_desc->Name,
                                "kernel32.dll") == 0)
                {
                void **thunk;
    ...
    (defined(WINNEWMOD) && !defined(__64BIT__))
                     ...
                     thunk++)
                {
                    ...
                    else
                    {
                        ...
                        int i;
                        typedef struct { char *name; unsigned long val; }
                            thunkentry;
                        static thunkentry stdthunkarr[] = {
STDTHUNKLIST
                        { NULL, 0 }
                        };

#if ZSHIM
                        static thunkentry zshimthunkarr[] = {
ZSHIMTHUNKLIST
                        { NULL, 0 }
                        };
#endif

                        static thunkentry *thunkiter = stdthunkarr;
                        ...
                        if (exe64)
                        {
                            thunkiter = zshimthunkarr;
                        }
                        else
                        {
                            thunkiter = stdthunkarr;
                        }
#endif
                        ...
                        /* printf("hintname is X%sX\n", hintname); */
                        ti = 0;
                        while (thunkiter[ti].name != NULL)
                        {
#ifdef CASEINS
                            if (ins_strncmp(thunkiter[ti].name, hintname, 8) == 0)
#else
                            if (strcmp(thunkiter[ti].name, hintname) == 0)
#endif
                            {
#ifdef ZSHIM
                                if (exe64)
...
#ifdef CASEINS
                        else if (ins_strncmp((char *)hintname,
                                            "__getmainargs",
                                            8) == 0)
#else
                        else if (strcmp((char *)hintname, "__getmainargs") == 0)
...
#ifdef CASEINS
                        else if (ins_strcmp(hintname, "exit") == 0)
#else
                        else if (strcmp(hintname, "exit") == 0)
#endif
                        {
...
#if defined(__M68K__) || defined(__MF32__)
#ifdef CASEINS
                        else if (ins_strncmp(hintname, "__iob_func", 8) == 0)
#else
                        else if (strcmp(hintname, "__iob_func") == 0)
#endif
                        {
...
#if defined(W32DLL) || defined(W64DLL) \
    || (defined(WINNEWMOD) && !defined(NOKERNEMUL))
                ...
                for (thunk = (void *)(exeStart + (import_desc->FirstThunk));
                     *thunk != NULL;
                     thunk++)
                {
                    if (((ptrdiff_t)(*thunk)) & 0x80000000UL)
                    {
                        /* Bit 31 set, import by ordinal. */
                        printf("ordinal import not supported\n");
                        return (2);
                    ...
                    {
                        /* Import by name. */
                        unsigned char *hintname = exeStart
                            + (((ptrdiff_t)(*thunk)) & 0x7fffffffUL);
                        int i;
...
STDW32THUNKLIST
                        ...
                        {
#ifdef CASEINS
                            if (ins_strncmp(thunkarr[ti].name, hintname, 8) == 0)
#else
                ...
                /* this one is used by the PDOS/386 kernel
                   - it can probably be merged with others - they
                     can likely all be merged */
...
#ifdef ZSHIM
    /* see below for masked out code */
    printf("DLLs not supported\n");
    return (2);
#endif
                         ...
                         unsigned long e_lfanew)
{
    ...
    struct object_table_entry_internal *object_table;
    struct object_page_table_entry_internal *object_page_table;
    ...
    lx_hdr_p = (void *)hdr;
#ifdef NEEDCBIG
    lx_hdr_p->ObjectTableOffsetHdr = flip4(lx_hdr_p->ObjectTableOffsetHdr);
    lx_hdr_p->ObjectPageTableOffsetHdr
        = flip4(lx_hdr_p->ObjectPageTableOffsetHdr);
    lx_hdr_p->DataPagesOffset = flip4(lx_hdr_p->DataPagesOffset);
    lx_hdr_p->NumberOfObjectsInModule
        = flip4(lx_hdr_p->NumberOfObjectsInModule);
    lx_hdr_p->PageSize = flip4(lx_hdr_p->PageSize);
    lx_hdr_p->FixupPageTableOffsetHdr
        = flip4(lx_hdr_p->FixupPageTableOffsetHdr);
    lx_hdr_p->FixupRecordTableOffsetHdr
        = flip4(lx_hdr_p->FixupRecordTableOffsetHdr);
    lx_hdr_p->EipObjectIndex = flip4(lx_hdr_p->EipObjectIndex);
    lx_hdr_p->Eip = flip4(lx_hdr_p->Eip);
#endif
...
#ifdef NEEDCBIG
    base_address = flip4(base_address);
#endif

    {
        struct object_table_entry_internal *obj;
        size_t highest_addr = 0;

        obj = object_table + lx_hdr_p->NumberOfObjectsInModule - 1;
#ifdef NEEDCBIG
        obj->RelocationBaseAddress = flip4(obj->RelocationBaseAddress);
        obj->VirtualSize = flip4(obj->VirtualSize);
#endif
    ...
    for (i = 0; i < lx_hdr_p->NumberOfObjectsInModule; i++) {
...
#ifdef NEEDCBIG
        object_table[i].RelocationBaseAddress
            = flip4(object_table[i].RelocationBaseAddress);
        object_table[i].PageTableIndex = flip4(object_table[i].PageTableIndex);
        object_table[i].NumberOfPageTableEntries
            = flip4(object_table[i].NumberOfPageTableEntries);
#endif
        ...
        {
#ifdef NEEDCBIG
            object_page_table[j].Flags = flip2(object_page_table[j].Flags);
            object_page_table[j].PageDataOffset
                = flip4(object_page_table[j].PageDataOffset);
            object_page_table[j].DataSize
                = flip2(object_page_table[j].DataSize);
#endif
    ...
    if (lx_hdr_p->FixupPageTableOffsetHdr
        && lx_hdr_p->FixupRecordTableOffsetHdr)
    {
        unsigned long *fixup_offset_table;
        ...
        for (i = 0; i < lx_hdr_p->NumberOfObjectsInModule; i++)
        {
                 ...
                 j++)
            {
                ...
                if (j == 0)
                {
                    fixup_offset_table[j] = flip4(fixup_offset_table[j]);
                }
                fixup_offset_table[j + 1] = flip4(fixup_offset_table[j + 1]);
#endif
                ...
                while (corr < corr_start + fixup_offset_table[j + 1])
                {
                    if (corr[0] == 0x08)
                    {
                        ...
                        if (ord == 234)
                        {
                            *(unsigned int *)(src_page + zapoffs) = (unsigned int)
                        ...
                        {
                            *(unsigned int *)(src_page + zapoffs) = (unsigned int)
                                ((unsigned char *)DosGetInfoBlocks
#ifndef __MF32__
                                 - (src_page + zapoffs + 4)
#endif
                                );
    ...
    {
        struct object_table_entry_internal *eip_obj;
