...
#define COPY_SIGNED(struct_name, field_name, bytes) \
 bytearray_signed_read_##bytes##_bytes (&struct_name##_internal->field_name, struct_name##_file->field_name, LITTLE_ENDIAN)
...
void read_struct_symbol_table_entry (struct symbol_table_entry_internal *symbol_table_entry_internal, const void *memory)
{
    ...
    COPY_SIGNED(symbol_table_entry, SectionNumber, 2);
