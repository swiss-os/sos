...
static unsigned short get_Machine (void)
{
    switch (ld_state->target_machine) {
        ...
        case LD_TARGET_MACHINE_LOONGARCH: return IMAGE_FILE_MACHINE_LOONGARCH64;
...
{
    flag_int flags = SECTION_FLAG_LOAD;

    if (Characteristics
        & (IMAGE_SCN_CNT_CODE
           | IMAGE_SCN_CNT_INITIALIZED_DATA
           | IMAGE_SCN_CNT_UNINITIALIZED_DATA)) flags |= SECTION_FLAG_ALLOC;
    ...
    if (Characteristics & IMAGE_SCN_CNT_UNINITIALIZED_DATA) flags &= ~SECTION_FLAG_LOAD;
                                        ...
                                        struct section_part *part)
{
    ...
    switch (input_reloc->Type) {
        ...
        case IMAGE_REL_ARM64_PAGEOFFSET_12L:
            /* COFF, unlike ELF, has only one relocation type for "str"-like instructions (str, ldr...),
            ...
            if (part->content[reloc->offset + 3] == 0x39) {
                ...
                ld_internal_error_at_source (__FILE__, __LINE__,
                                             "%s: unknown instruction '%02x %02x %02x %02x' used with IMAGE_REL_ARM64_PAGEOFFSET_12L",
                                             part->of->filename,
...
void coff_write (const char *filename)
{
    ...
    coff_hdr.SizeOfOptionalHeader = (use_pe32_plus
        ...
        if (coff_symbol->SectionNumber == IMAGE_SYM_UNDEFINED) {
            if (symbol->value) {
                ...
                if (!old_symbol || symbol_is_undefined (old_symbol)) {
                     ...
                     * but at most SectionAlignment to avoid wasting memory
                                    ...
                                    const char *filename)
{
    ...
    part = section_part_new (section, of);
    subsection_append_section_part (subsection, part);
    part->content_size = use_pe32_plus ? 8 : 4;
    part->content = xmalloc (part->content_size);
    memset (part->content, 0, part->content_size);
    ...
    if (ImportType == IMPORT_CODE) {
        ...
        if (ld_state->target_machine == LD_TARGET_MACHINE_I386
            ...
            part->content = xmalloc (part->content_size);
            memcpy (part->content,
                    "\x0C\x00\x00\x1A"      /* la.global $r12,iat*/
                    "\x8C\x01\xC0\x28"
                    "\x80\x01\x00\x4C"      /* jr $r12 */
                    /* "\x00\x00\x40\x03" */ /* nop - unused */
                    , part->content_size);

        } else if (ld_state->target_machine == LD_TARGET_MACHINE_MAINFRAME
                   && ld_state->force_bits == 64) {
            part->content_size = 24;
            part->content = xmalloc (part->content_size);

            memcpy (part->content,
         /*  0 */   "\x05\xF0"         /* BALR R15,R0 */
                                       /* USING *,R15 */
         /*  2 */   "\xE3\xF0\xF0\x0E" /* LG R15,XXX */
                    "\x00\x04"
                    /* This is an RXY-a instruction, and the x'04' is a
                       fixed part of the instruction, and the x'00' before
                       it is a high displacement - giving a 20-bit displacement
                       instead of the traditional 16-bits */
                    /* Also note that the displacement is E, not 10, because
                       the BALR set R15 to point to the next instruction, ie
                       offset 2, so displacements are relative to 2, not 0 */

         /*  8 */   "\xE3\xF0\xF0\x00" /* LG R15,0(,R15) */
                    "\x00\x04"

         /* 14 */   "\x07\xFF"         /* BR R15 */
         /* 16 */   /* "\x07\x00" */         /* NOPR R0 */
                    /* DS 0D */
         /* 16 */   "\x00\x00\x00\x00" "\x00\x00\x00\x00" /* XXX DC 8X'00'*/,
                    24);
               /* if the instruction count is increased,
                  change the content_size = 24 above */
               /* if the offset to be zapped is changed, change below
                  offset = 16 */
        } else if (ld_state->target_machine == LD_TARGET_MACHINE_MAINFRAME) {
            part->content_size = 16;
            part->content = xmalloc (part->content_size);
            memcpy (part->content,
         /*  0 */   "\x05\xF0"         /* BALR R15,R0 */
                                       /* USING *,R15 */
         /*  2 */   "\x58\xF0\xF0\x0A" /* L R15,XXX */
         /*  6 */   "\x58\xF0\xF0\x00" /* L R15,0(,R15) */
         /* 10 */   "\x07\xFF"         /* BR R15 */
         /* 12 */   /* "\x07\x00" */         /* NOPR R0 */
         /* 12 */   "\x00\x00\x00\x00" /* XXX DC F'0'*/,
                    16);
               /* the offset to be zapped can be seen below as
                  offset = 12 */
               /* and if the instruction count is increased,
                  change the content_size = 16 above */
        } else {
            ld_internal_error_at_source (__FILE__, __LINE__,
        ...
        /* some platforms have 2 places that need to be zapped */
        ...
        if (ld_state->target_machine == LD_TARGET_MACHINE_X64) {
            ...
            relocs[0].howto = &reloc_howtos[RELOC_TYPE_LOONGARCH_PCALA_HI20];
            relocs[0].offset = 0;
            relocs[1].symbol = &of->symbol_array[0];
            relocs[1].howto = &reloc_howtos[RELOC_TYPE_LOONGARCH_PCALA_LO12];
            relocs[1].offset = 4;
        } else if (ld_state->target_machine == LD_TARGET_MACHINE_MAINFRAME
                   && ld_state->force_bits == 64) {
            relocs[0].howto = &reloc_howtos[RELOC_TYPE_64];
            relocs[0].offset = 16;
        } else if (ld_state->target_machine == LD_TARGET_MACHINE_MAINFRAME) {
            relocs[0].howto = &reloc_howtos[RELOC_TYPE_32];
            relocs[0].offset = 12;
