...
static const struct long_option long_options[] = {
    ...
    { STR_AND_LEN("64"), LD_OPTION_FORCE_BITS64, OPTION_NO_ARG},
    { NULL, 0, 0}
...
static void print_help (void)
{
    ...
    printf ("  --oformat FORMAT            Create an output file in format FORMAT (default %s)\n",
            "coff");
    printf ("                                Supported formats are: amiga, a.out, atari, binary, cms,\n");
    printf ("                                                       coff, elf, lx, macho, mvs, mts,\n");
    printf ("                                                       musicsp, vse, xmit, z390\n");
    ...
    printf ("  --64                        Force 64-bit executable format\n");
    ...
    switch (option_index) {
        ...
        case LD_OPTION_OFORMAT:
            if (strcmp (arg, "amiga") == 0) {
                ld_state->oformat = LD_OFORMAT_AMIGA;
                ...
                ld_state->oformat = LD_OFORMAT_MACHO;
            } else if (strcmp (arg, "mvs") == 0) {
                ld_state->oformat = LD_OFORMAT_MVS;
            } else if (strcmp (arg, "mts") == 0) {
                ld_state->oformat = LD_OFORMAT_MTS;
            } else if (strcmp (arg, "musicsp") == 0) {
                ld_state->oformat = LD_OFORMAT_MUSICSP;
            } else if (strcmp (arg, "vse") == 0) {
                ld_state->oformat = LD_OFORMAT_VSE;
            } else if (strcmp (arg, "xmit") == 0) {
                ld_state->oformat = LD_OFORMAT_XMIT;
            } else if (strcmp (arg, "z390") == 0) {
                ld_state->oformat = LD_OFORMAT_Z390;
            } else {
...
char **ld_parse_args (int argc, char **argv, int start_index)
{
    ...
    struct options_with_use options_with_use[7];
    ...
    options_with_use[2].options = elf_get_long_options ();
    options_with_use[2].use = &elf_use_option;
    options_with_use[3].options = lx_get_long_options ();
    options_with_use[3].use = &lx_use_option;
    options_with_use[4].options = macho_get_long_options ();
    options_with_use[4].use = &macho_use_option;
    options_with_use[5].options = mainframe_get_long_options ();
    options_with_use[5].use = &mainframe_use_option;
    options_with_use[6].options = NULL;
    options_with_use[6].use = NULL;
