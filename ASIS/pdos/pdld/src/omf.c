...
static unsigned int offset32_loc;
    ...
    while (pos < file + file_size) {
        ...
        if (record_type == RECORD_TYPE_THEADR) {
            ...
            if (ld_state->oformat == LD_OFORMAT_AOUT) {
                /* Map OMF input sections
                 * to a.out output sections
                 * so non-empty output executables
                 * are produced.
                 */
                const char *name;
                ...
                section = section_find_or_make (name);
            } else {
            ...
            while (subrec != end) {
                if (subrec[0] & 0x80) {
                    ...
                    locat = subrec[0];
                    /* LOC 3 is 32-bit Long Pointer
                     * (16-bit base : 16-bit offset)
                     * which can be converted to LOC 9
                     * but LOC 3 and LOC 9 should not
                     * be mixed.
                     */
                    if (((locat >> 2) & 0xf) == 3) {
                        if (offset32_loc != 3
                            && offset32_loc) {
                            ld_error ("%s: OMF LOC 3 and 9 inconsistency",
                                      filename);
                        }
                        offset32_loc = 3;
                        locat &= ~(0xf << 2);
                        locat |= 9 << 2;
                    } else if (((locat >> 2) & 0xf) == 9) {
                        if (offset32_loc != 9
                            && offset32_loc) {
                            ld_error ("%s: OMF LOC 3 and 9 inconsistency",
                                      filename);
                    ...
                    if (((locat >> 2) & 0xf) != 1
                        && ((locat >> 2) & 0xf) != 9) {
                        ld_internal_error_at_source (__FILE__, __LINE__,
                                                     "%s: only 16-bit offset and 32-bit offset OMF fixups are supported"
                                                     " (current LOC %u)",
                                                     filename, (locat >> 2) & 0xf);
                    }
