...
/* Needed only for calculating relocations, so it uses the same logic as link.c. */
    ...
    for (section = all_sections; section; section = section->next) {
        ...
        for (part = section->first_part; part; part = part->next) {
            ...
            for (i = 0; i < part->relocation_count; i++) {
                ...
                if (symbol_is_undefined (part->relocation_array[i].symbol)
                    && (symbol = find_symbol (part->relocation_array[i].symbol->name))
                    && symbol->part == &fake_lx_part_s) {
                    /* There used to be first condition
                     * "part->relocation_array[i].howto == &reloc_howtos[RELOC_TYPE_PC32]"
                     * but it is applicable only to original x86 LX,
                     * not to other machines because those can use different relocations
                    ...
                    if (symbol_is_undefined ((symbol = part->relocation_array[i].symbol))) {
                        if (symbol_is_undefined ((symbol = find_symbol (symbol->name)))) continue;
                    }
                    if (!symbol->part) continue; /* Absolute symbol. */

                    source_section = symbol->part->section;
                    bytearray_read_4_bytes (&field, part->content + part->relocation_array[i].offset, endianess);
                    value = field;
                               ...
                               size_t lx_hdr_offset)
{
    ...
    if (dll_inames_size) {
        ...
        ttgtchsmem(page_e_pos, dll_inames_size);
    }
}
                                              ...
                                              const char *dll_name)
{
    ...
    if (less_strict_mainframe_matching) {
        mainframe_symbol_name_prepare (symbol->name);
    }
