...
struct archive_member_header {
    char *name;
    unsigned long size;
    unsigned long name_size_in_member;
};
...
static int read_archive_member_header (unsigned char *file,
                                       size_t file_size,
                                       ...
                                       const char *archive_name)
{
    ...
    if (hdr->name[0] == '/' && isdigit ((unsigned char)hdr->name[1])) {
        unsigned long i = strtoul (hdr->name + 1, NULL, 10);

        if (!longnames->names) {
            ld_warn ("%s: archive member has longname but archive lacks longnames member",
                     archive_name);
        } else if (i >= longnames->size) {
            free (hdr->name);
            ld_error ("$s: longname index %lu larger than longnames size %lu",
                      archive_name, i, longnames->size);
            return 1;
        } else {
            const char *p, *p2;
    ...
    } else if (strncmp ("#1/", hdr->name, 3) == 0) {
        /* Another way of having longnames,
         * the decimal number after "#1/"
         * is number of bytes inside
         * the archive member content
         * that contain the name.
         * The bytes with name
         * must be skipped later on
         * when using the content.
         */
        char *p;
        unsigned long name_size;

        name_size = strtoul (hdr->name + 3, &p, 10);
        if (*p) {
            ld_error ("%s: invalid archive member longname '%s'",
                      archive_name, hdr->name);
            free (hdr->name);
            return 1;
        }

        if (name_size > hdr->size) {
            ld_error ("%s: invalid archive member longname size %lu",
                      archive_name, name_size);
            free (hdr->name);
            return 1;
        }
        ...
        free (hdr->name);
        hdr->name = xstrndup ((char *)pos, name_size);
        hdr->name_size_in_member = name_size;
    }
...
static struct lm_offset_name_entry *read_symdef_sorted (unsigned char *file,
                                                        size_t file_size,
                                                        const char *archive_name,
                                                        unsigned long *NumberOfSymbols_p)
{
    /* SYMDEF SORTED is a different
     * linker member format
     * with all fields little endian:
     * "__.SYMDEF SORTED" name (already skipped)
     * Number of bytes of the following array, 4 bytes
     * Array of 8-byte structs with format:
     *  Symbol name offset in string table, 4 bytes
     *  File offset to archive member containg symbol, 4 bytes
    ...
    CHECK_READ (pos, 4);
    {
        ...
        bytearray_read_4_bytes (&num_bytes, pos, LITTLE_ENDIAN);
        if (num_bytes % 8) {
            ld_error ("%s: __.SYMDEF SORTED first field value not divisible by 8",
                      archive_name);
        }
        NumberOfSymbols = num_bytes / 8;
    }
    pos += 4;

    offset_name_table = xmalloc (sizeof (*offset_name_table) * NumberOfSymbols);
    string_table_pos = pos + NumberOfSymbols * 8 + 4;
    ...
    for (i = 0; i < NumberOfSymbols; i++) {
        ...
        CHECK_READ (pos, 8);
        bytearray_read_4_bytes (&str_offset, pos, LITTLE_ENDIAN);
        pos += 4;
        bytearray_read_4_bytes (&offset_name_table[i].offset, pos, LITTLE_ENDIAN);
        pos += 4;
        
        str = string_table_pos + str_offset;
        CHECK_READ (str, 1);
        offset_name_table[i].name = (char *)str;
    }
                                ...
                                const struct archive_longnames *longnames)
{
    ...
    if (read_archive_member_header (file, file_size, pos, &hdr, longnames, archive_name)) return INPUT_FILE_ERROR;
    memcpy (pos, ALREADY_READ_NAME, sizeof (ALREADY_READ_NAME));
    pos += SIZEOF_struct_IMAGE_ARCHIVE_MEMBER_HEADER_file;
    pos += hdr.name_size_in_member;
...
static void read_archive (unsigned char *file, size_t file_size, const char *archive_name)
{
    ...
    CHECK_READ (pos, SIZEOF_struct_IMAGE_ARCHIVE_MEMBER_HEADER_file);
    if (read_archive_member_header (file, file_size, pos, &hdr, &longnames, archive_name)) return;
    pos += SIZEOF_struct_IMAGE_ARCHIVE_MEMBER_HEADER_file;
    pos += hdr.name_size_in_member;

    if (strcmp (hdr.name, IMAGE_ARCHIVE_LINKER_MEMBER_Name) == 0) {
        offset_name_table = read_linker_member (pos,
                                                MIN (hdr.size, file_size - (pos - file)),
                                                archive_name,
                                                &NumberOfSymbols);
    } else if (strcmp (hdr.name, "__.SYMDEF SORTED") == 0) {
        offset_name_table = read_symdef_sorted (pos,
                                                MIN (hdr.size, file_size - (pos - file)),
                                                archive_name,
                                                &NumberOfSymbols);
    } else {
