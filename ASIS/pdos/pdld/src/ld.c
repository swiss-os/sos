...
int main (int argc, char **argv)
{
    ...
    if (ld_state->oformat != LD_OFORMAT_BINARY) {
        /* There is no need to remove empty sections
    ...
    if (ld_state->oformat == LD_OFORMAT_AMIGA) {
        ...
        macho_write (ld_state->output_filename);
    } else if (ld_state->oformat == LD_OFORMAT_MVS) {
        mvs_write (ld_state->output_filename);
    } else if (ld_state->oformat == LD_OFORMAT_MTS) {
        mts_write (ld_state->output_filename);
    } else if (ld_state->oformat == LD_OFORMAT_MUSICSP) {
        musicsp_write (ld_state->output_filename);
    } else if (ld_state->oformat == LD_OFORMAT_VSE) {
        vse_write (ld_state->output_filename);
    } else if (ld_state->oformat == LD_OFORMAT_XMIT) {
        xmit_write (ld_state->output_filename);
    } else if (ld_state->oformat == LD_OFORMAT_Z390) {
        z390_write (ld_state->output_filename);
    }
