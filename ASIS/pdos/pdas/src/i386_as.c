...
void machine_dependent_init (void)
{
    ...
    if (state->format == AS_FORMAT_COFF) {
        coff_x86_set_bits (bits);
    } else if (state->format == AS_FORMAT_ELF) {
        elf_x86_set_bits (bits);
        if (bits == 64) use_rela_relocations = 1;
    }
}
    ...
    for (operand = 0; operand < instruction.operands; operand++) {
        if (operand_type_check (instruction.types[operand], imm)) {
            if (instruction.imms[operand]->type == EXPR_TYPE_CONSTANT) {
                ...
                /* MOV 32-bit immediate to 64-bit register
                 * sign extends the immediate,
                 * so imm64 must be used instead of imm32.
                 * (Signed 32-bit immediates are covered by imm32s.)
                 */
                if (guessed_suffix == QWORD_SUFFIX) {
                    instruction.types[operand].imm32 = 0;
                }
            }
        }
    }
...
void machine_dependent_apply_fixup (struct fixup *fixup, value_t value)
{
    ...
    if (fixup->add_symbol == NULL) {
        ...
        /* The addend is put into
         * the addend field of RELA later.
         */
        fixup->add_number = value;
        value = 0;
    }
...
static void set_syntax (char **pp, int set_intel_syntax)
{
    ...
    *pp = skip_whitespace (*pp);
    ...
    if (set_intel_syntax) {
        /* It might be better to check whether symbol names
         * have prefix or not and when they do not,
         * use the second logic and otherwise the first.
         * (32-bit cdecl has "_" prefix before symbol names,
         *  so no conflict would occur.)
         */
#if 0
