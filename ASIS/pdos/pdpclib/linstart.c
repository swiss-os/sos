...
#ifdef __MACOS__
extern int __start(int argc, char **argv, char **envp);
#else
...
/* Actually, I suspect it is simply the OS setting that
   standard for MacOS ARM64 - not using the stack, and
   passing arguments in registers instead. */

int _start(int argc, char **argv, char **envp)
{
    int rc;

    rc = __start(argc, argv, envp);
    __exita(rc);
    return (rc);
}
