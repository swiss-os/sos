...
# We should really switch this to intel syntax and noprefix
...
# Note that the entry point can't be _start in linstart.c for some reason
# Presumably the stack is wonky or something like that. Or some guard
# space is required. So at time of writing, pdas would fail - unoptimized
# or optimized - in different places - if we don't have this as our entry
# point. In addition, I think we ideally need a pointer to the stack
# anyway, as the C code may need to inspect that later (normally it
# doesn't need to though - if the /proc/<pid> is there).

# Also note that you need to keep the stack 16-byte aligned, so if you
# push registers etc, make sure it's an even number, or compensate for that.
...
# Normally not expecting _start to return
mov %rax,%rdi
call __exita

# shouldn't get here, but if we do, loop instead of executing
# random code
jjj: jmp jjj
...
# probably don't need these pushes and pops
push %r10
push %r10
